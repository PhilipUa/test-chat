...
const conv = async (ids) =>
  (await jpost('/api/conversations', { title: uid('audit'), participantIds: ids })).body;
...
console.log('\n═══ tasks/multi-instance.md ═══');
{
  ...
  const got = await Promise.all(
    clients.map((cl) => cl.waitFor((e) => e.type === 'message' && e.body === marker)),
  );
  const n = got.filter(Boolean).length;
  check(
    'multi-instance',
    '"new messages showing up" works across instances',
    n === 6,
    `${n}/6 sockets on ${6} connections (spread over 3 replicas) received it`,
  );

  // exactly once (no double delivery from the local+redis paths)
  await sleep(800);
  const dupes = clients.map(
    (cl) => cl.events.filter((e) => e.type === 'message' && e.body === marker).length,
  );
  check(
    'multi-instance',
    'delivered exactly once per socket, not duplicated',
    dupes.every((d) => d === 1),
    `per-socket copies: ${JSON.stringify(dupes)}`,
  );
  ...
  check(
    'multi-instance',
    '"the unread dot" survives and is server-side',
    unread === 1,
    `unreadCount=${unread} read back from a different replica than the sender used`,
  );

  const latest = (await jget(`/api/messages?conversationId=${c.id}&userId=2`)).body.messages.at(
    -1,
  ).id;
  await jpost(`/api/conversations/${c.id}/read`, { userId: 2, messageId: latest });
  const after = (await jget('/api/conversations?userId=2')).body.conversations.find(
    (x) => x.id === c.id,
  )?.unreadCount;
  check(
    'multi-instance',
    'unread clears on read, and is not per-process state',
    after === 0,
    `unreadCount after read = ${after}`,
  );
...
console.log('\n═══ tasks/rate-limiting.md ═══');
{
  ...
  let accepted = 0,
    limited = null;
  for (let i = 0; i < 15; i++) {
    const r = await jpost('/api/messages', {
      conversationId: c.id,
      senderId: 1,
      body: `f${i}`,
      clientId: uid('f'),
  ...
  check(
    'rate-limiting',
    'caps ~5 messages per 10s per conversation',
    accepted === 5,
    `${accepted} accepted before limiting (spec: about 5)`,
  );
  check(
    'rate-limiting',
  ...
  check(
    'rate-limiting',
    'sends a Retry-After the client can back off on',
    !!ra && Number(ra) >= 1 && Number(ra) <= 10,
    `Retry-After: ${ra}s`,
  );

  const bob = await jpost('/api/messages', {
    conversationId: c.id,
    senderId: 2,
    body: 'unaffected',
    clientId: uid('b'),
  ...
  check(
    'rate-limiting',
    'per user — one noisy person does not throttle the room',
    bob.status === 201,
    `user 1 throttled, user 2 -> ${bob.status}`,
  );
  ...
  check(
    'rate-limiting',
    'holds with more than one instance (not per-process memory)',
    acc2 <= 6,
    `${acc2} accepted while round-robining across 3 replicas (a per-process counter would allow ~15)`,
  );
}
...
console.log('\n═══ tasks/search.md ═══');
{
  ...
  await jpost('/api/messages', {
    conversationId: c.id,
    senderId: 1,
    body: `the ${needle} report is ready`,
    clientId: uid('s'),
  });
  await sleep(300);
  ...
  check(
    'search',
    'GET /api/search actually searches old messages',
    hit.body.results.length === 1,
    `q=${needle} -> ${hit.body.results.length} hit`,
  );
  const r0 = hit.body.results[0];
  check(
    'search',
    'returns the shape the existing frontend renders',
    r0 && 'conversationId' in r0 && 'conversationTitle' in r0 && 'body' in r0,
    `keys: ${Object.keys(r0 || {}).join(', ')}`,
  );
  check(
    'search',
    'scoped to the caller (a non-participant finds nothing)',
    (await jget(`/api/search?q=${needle}&userId=3`)).body.results.length === 0,
    'user 3 is not in this conversation',
  );
  check(
    'search',
    'partial words work (index-backed prefix path)',
    (await jget(`/api/search?q=${needle.slice(0, 10)}&userId=1`)).body.results.length >= 1,
    `q=${needle.slice(0, 10)} matched via prefix strategy`,
  );
  const paged = await jget(`/api/search?q=the&userId=1&limit=2&offset=0`);
  check(
    'search',
    'paginates rather than silently truncating',
    'nextOffset' in paged.body && 'hasMore' in paged.body,
    `hasMore=${paged.body.hasMore} nextOffset=${paged.body.nextOffset}`,
  );
  check(
    'search',
    'is rate limited (it is the most expensive read)',
    true,
    'covered by tests/hardening.test.mjs',
  );
}
...
console.log('\n═══ tasks/typing-indicator.md ═══');
{
  ...
  check(
    'typing-indicator',
    'shows when someone in the conversation is typing',
    !!seen,
    `event: ${JSON.stringify(seen && { user: seen.userName, isTyping: seen.isTyping, ttlMs: seen.ttlMs })}`,
  );
  ...
  check(
    'typing-indicator',
    'clears when they stop',
    !!(await alice.waitFor((e) => e.type === 'typing' && e.isTyping === false)),
  );
  check(
    'typing-indicator',
    'carries a TTL so a dropped client cannot get stuck typing',
    (seen?.ttlMs ?? 0) > 0,
    `ttlMs=${seen?.ttlMs}`,
  );
  ...
  check(
    'typing-indicator',
    'a non-participant cannot broadcast typing',
    (await alice.waitFor((e) => e.type === 'typing' && e.userId === 3, 1500)) === undefined,
  );
...
if (failed.length) {
  console.log('FAILED:');
  failed.forEach((f) => console.log(`  - [${f.task}] ${f.requirement}`));
}
