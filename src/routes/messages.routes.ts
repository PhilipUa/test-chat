...
import { listMessagesQuerySchema, messagePayloadSchema } from '../validation/schemas.ts';
 ...
 *   validateBody         400 for a missing or oversized body — the schema is the boundary
...
messagesRouter.post(
  '/',
  requireParticipant({
    actor: fromBody('senderId'),
    conversation: fromBody('conversationId'),
    actorField: 'senderId',
  }),
  validateBody(messagePayloadSchema, (res, payload) => {
    res.locals.newMessage = payload;
  }),
  rateLimit({
    rule: config.rateLimit.send,
    key: (_req, res) => `send:${actorId(res)}:${conversationId(res)}`,
 ...
 *
 * Metered on the shared `reads` bucket. A page is bounded, so this isn't the fan-out search is,
...
messagesRouter.get(
  ...
  validateQuery(listMessagesQuerySchema, (res, query) => {
    res.locals.messagesQuery = query;
  }),
  rateLimitReads,
